...
describe('balanceService auto relogin', () => {
  ...
  it('auto topups the configured target site when a manual refresh finds balance below threshold', async () => {
    selectAllMock.mockReturnValue([
      {
        accounts: {
          id: 50,
    ...
    adapterMock.getBalance
      .mockResolvedValueOnce({ balance: 9.5, used: 0, quota: 9.5 })
      .mockResolvedValueOnce({ balance: 5, used: 0, quota: 5 });
    undiciFetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ ok: true, message: '领取成功' }),
    });

    const { refreshBalance } = await import('./balanceService.js');
    const result = await refreshBalance(50);

    expect(result).toEqual({ balance: 5, used: 0, quota: 5 });
    expect(adapterMock.getBalance).toHaveBeenCalledTimes(2);
    expect(undiciFetchMock).toHaveBeenCalledTimes(1);
    expect(undiciFetchMock).toHaveBeenCalledWith(
      'https://ye.99dun.cc/api/topup',
      expect.objectContaining({
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ account: 'BloodVeil1@tg.user' }),
  ...
  it('records an error event when target-site auto topup fails', async () => {
    selectAllMock.mockReturnValue([
      {
        accounts: {
          id: 50,
    ...
    adapterMock.getBalance.mockResolvedValueOnce({ balance: 8, used: 0, quota: 8 });
    undiciFetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ ok: false, message: '当前不满足领取条件。' }),
    });

    const { refreshBalance } = await import('./balanceService.js');
    const result = await refreshBalance(50);

    expect(result).toEqual({ balance: 8, used: 0, quota: 8 });
    expect(adapterMock.getBalance).toHaveBeenCalledTimes(1);
    expect(undiciFetchMock).toHaveBeenCalledTimes(1);
    expect(insertValuesMock).toHaveBeenCalledWith(expect.objectContaining({
      type: 'balance',
  ...
  it('does not auto topup non-target sites even when balance is below threshold', async () => {
    selectAllMock.mockReturnValue([
      {
        accounts: {
          id: 88,
    ...
    adapterMock.getBalance.mockResolvedValueOnce({ balance: 3, used: 0, quota: 3 });

    const { refreshBalance } = await import('./balanceService.js');
    const result = await refreshBalance(88);

    expect(result).toEqual({ balance: 3, used: 0, quota: 3 });
    expect(undiciFetchMock).not.toHaveBeenCalled();
  });

  it('skips repeated auto topup attempts during cooldown window', async () => {
    ...
    selectAllMock.mockReturnValue([
      {
        accounts: {
          ...
          extraConfig: JSON.stringify({
            autoTopup: {
              lastAttemptAt: 1_800_000_000_000 - 60_000,
            },
          }),
        },
        sites: {
    ...
    adapterMock.getBalance.mockResolvedValueOnce({ balance: 2, used: 0, quota: 2 });

    const { refreshBalance } = await import('./balanceService.js');
    const result = await refreshBalance(50);

    expect(result).toEqual({ balance: 2, used: 0, quota: 2 });
    expect(undiciFetchMock).not.toHaveBeenCalled();
    nowSpy.mockRestore();
  });

  it('shares cooldown across accounts on the same target site to avoid duplicate topups', async () => {
    ...
    selectAllMock
      ...
      .mockReturnValueOnce([
        { id: 50, extraConfig: JSON.stringify({ autoTopup: { lastAttemptAt: 1_900_000_000_000 - 30_000 } }) },
        { id: 52, extraConfig: null },
      ]);

    adapterMock.getBalance.mockResolvedValueOnce({ balance: 2, used: 0, quota: 2 });

    const { refreshBalance } = await import('./balanceService.js');
    const result = await refreshBalance(52);
  ...
  it('prevents duplicate concurrent topups for the same target site', async () => {
    selectAllMock
      .mockReturnValueOnce([
        {
          accounts: {
      ...
      .mockReturnValueOnce([
        { id: 50, siteId: 51, extraConfig: null },
      ])
      .mockReturnValueOnce([
        {
          accounts: {
    ...
    adapterMock.getBalance
      .mockResolvedValueOnce({ balance: 2, used: 0, quota: 2 })
      .mockResolvedValueOnce({ balance: 2, used: 0, quota: 2 });
    ...
    const { refreshBalance } = await import('./balanceService.js');
    const firstPromise = refreshBalance(50);
    await new Promise((resolve) => setTimeout(resolve, 0));
    const secondPromise = refreshBalance(52);
    const [first, second] = await Promise.all([firstPromise, secondPromise]);

    expect(first).toEqual({ balance: 2, used: 0, quota: 2 });
    expect(second).toEqual({ balance: 2, used: 0, quota: 2 });
    expect(undiciFetchMock).toHaveBeenCalledTimes(1);
  });
});
