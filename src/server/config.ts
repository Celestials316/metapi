...
export function buildConfig(env: NodeJS.ProcessEnv) {
  ...
  return {
    ...
    proxyStreamIdleTimeoutMs: Math.max(0, Math.trunc(parseNumber(env.PROXY_STREAM_IDLE_TIMEOUT_MS, 30_000))),
    codexWebsocketIdleTimeoutMs: Math.max(0, Math.trunc(parseNumber(env.CODEX_WEBSOCKET_IDLE_TIMEOUT_MS, 30_000))),
